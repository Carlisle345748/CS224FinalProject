...
class AdaptiveBatchSampler(Sampler[int]):
	...
	def __init__(self,
		...
		if resume_from_checkpoint:
			self.sim = Similarity(cache_file=os.path.join(resume_from_checkpoint, "sim.npz"))
			...
			if sim_cache_file:
				self.sim = Similarity(cache_file=sim_cache_file)
			else:
	...
	def get_add(self, batch_size, current_dataset, pair_remove, remain_dataset, search_range=100,
		...
		return candidates[top_k.indices.cpu()], top_k.values.cpu().numpy()
